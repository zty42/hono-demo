// .wrangler/tmp/bundle-2ezebD/checked-fetch.js
    ...
    if (/\*$/.test(path)) {
      ...
      if (method === METHOD_NAME_ALL) {
        Object.keys(middleware2).forEach((m) => {
          var _a2;
          (_a2 = middleware2[m])[path] || (_a2[path] = findMiddleware(middleware2[m], path) || findMiddleware(middleware2[METHOD_NAME_ALL], path) || []);
        });
      } else {
        (_a = middleware2[method])[path] || (_a[path] = findMiddleware(middleware2[method], path) || findMiddleware(middleware2[METHOD_NAME_ALL], path) || []);
      }
      Object.keys(middleware2).forEach((m) => {
...
var createPost = async (KV, param) => {
  if (!(param && param.title && param.body))
    return;
  const id = crypto.randomUUID();
  const newPost = { id, title: param.title, body: param.body };
  await KV.put(generateID(id), JSON.stringify(newPost));
  return newPost;
};
var updatePost = async (KV, id, param) => {
  if (!(param && param.title && param.body))
    return false;
...
api.get("/posts", async (c) => {
  const posts = await getPosts(c.env.BLOG_EXAMPLE);
  return c.json({ posts, ok: true });
});
api.post("/posts", async (c) => {
  const param = await c.req.json();
  const newPost = await createPost(c.env.BLOG_EXAMPLE, param);
  if (!newPost) {
...
api.get("/posts/:id", async (c) => {
  ...
  const post = await getPost(c.env.BLOG_EXAMPLE, id);
  if (!post) {
...
api.put("/posts/:id", async (c) => {
  ...
  const param = await c.req.json();
  const success = await updatePost(c.env.BLOG_EXAMPLE, id, param);
...
api.delete("/posts/:id", async (c) => {
  ...
  const success = await deletePost(c.env.BLOG_EXAMPLE, id);
  return c.json({ ok: success });
});
var api_default = api;
...
var middleware = new Hono2();
middleware.use("*", prettyJSON());
middleware.use("/posts/*", async (c, next) => {
  if (c.req.method !== "GET") {
    const auth2 = basicAuth({ username: c.env.USERNAME, password: c.env.PASSWORD });
    return auth2(c, next);
  } else {
    await next();
  }
...
// .wrangler/tmp/bundle-2ezebD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-2ezebD/middleware-loader.entry.ts
