// .wrangler/tmp/bundle-n7OwDx/checked-fetch.js
...
api.get("/posts", async (c) => {
  const posts = await getPosts(c.env.memo);
  return c.json({ posts, ok: true });
});
api.post("/posts", async (c) => {
  const param = await c.req.json();
  const newPost = await createPost(c.env.memo, param);
  if (!newPost) {
...
api.get("/posts/:id", async (c) => {
  ...
  const post = await getPost(c.env.memo, id);
  if (!post) {
...
api.put("/posts/:id", async (c) => {
  ...
  const param = await c.req.json();
  const success = await updatePost(c.env.memo, id, param);
...
api.delete("/posts/:id", async (c) => {
  ...
  const success = await deletePost(c.env.memo, id);
  return c.json({ ok: success });
});
var api_default = api;
...
// .wrangler/tmp/bundle-n7OwDx/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-n7OwDx/middleware-loader.entry.ts
