// .wrangler/tmp/bundle-VTmlCO/checked-fetch.js
...
var getPosts = async (KV) => {
  const list = await KV.list({ prefix: PREFIX2 });
...
api.get("/posts", async (c) => {
  const posts = await getPosts(c.env.content);
  return c.json({ posts, ok: true });
});
api.post("/posts", async (c) => {
  const param = await c.req.json();
  const newPost = await createPost(c.env.content, param);
  if (!newPost) {
...
api.get("/posts/:id", async (c) => {
  ...
  const post = await getPost(c.env.content, id);
  if (!post) {
...
api.put("/posts/:id", async (c) => {
  ...
  const param = await c.req.json();
  const success = await updatePost(c.env.content, id, param);
...
api.delete("/posts/:id", async (c) => {
  ...
  const success = await deletePost(c.env.content, id);
  return c.json({ ok: success });
});
var api_default = api;
...
// .wrangler/tmp/bundle-VTmlCO/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VTmlCO/middleware-loader.entry.ts
